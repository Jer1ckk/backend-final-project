...
const getRandomDiscount = () =>
  [0, 10, 15, 20, 25, 30, 35, 40, 45, 50][Math.floor(Math.random() * 10)];
...
const generateRandomProduct = (
  ...
  const templates = productTemplates[subcategorySlug] || [
    "{adjective} Product",
  ];
  ...
  const salePrice =
    discount > 0 ? +(originalPrice * (1 - discount / 100)).toFixed(2) : null;
  ...
  if (subcategorySlug.includes("shoes")) {
    ...
    subcategorySlug.includes("glasses") ||
    subcategorySlug.includes("bag") ||
    subcategorySlug.includes("wallet") ||
    subcategorySlug.includes("hat")
  ) {
  ...
  const descriptions = [
    `${adjective} ${subcategorySlug.replace(
      "-",
      " "
    )} perfect for any occasion. Made with high-quality materials for comfort and durability.`,
    `Stylish and comfortable ${subcategorySlug.replace(
      "-",
      " "
    )} that combines fashion with functionality. Perfect addition to your wardrobe.`,
    `Premium quality ${subcategorySlug.replace(
      "-",
      " "
    )} designed for the modern individual. Versatile piece that goes with everything.`,
    `Essential ${subcategorySlug.replace(
      "-",
      " "
    )} featuring superior craftsmanship and attention to detail. A must-have for fashion enthusiasts.`,
    `Contemporary ${subcategorySlug.replace(
      "-",
      " "
    )} that offers both style and comfort. Perfect for everyday wear or special occasions.`,
  ];
  ...
  return {
    ...
    sku: `${categorySlug.substring(0, 1).toUpperCase()}${subcategorySlug
      .substring(0, 2)
      .toUpperCase()}${String(index).padStart(3, "0")}`,
    ...
    metaDescription: `Shop ${productName} at ${getRandomElement(
      brands
    )}. ${getRandomElement(descriptions).substring(0, 150)}...`,
  };
};
...
const seedDatabase = async () => {
  try {
    ...
    const productsPerSubcategory = Math.floor(
      targetProductCount / subcategoryArray.length
    );
    ...
    for (let i = 0; i < subcategoryArray.length; i++) {
      ...
      console.log(
        `  📦 Generating ${numProducts} products for ${subcategory.name}...`
      );
    ...
    for (let i = 0; i < allProducts.length; i += batchSize) {
      ...
      console.log(
        `✅ Created products ${i + 1} to ${Math.min(
          i + batchSize,
          allProducts.length
    ...
    const subcategoryCount = await Category.count({
      where: { parentId: { [require("sequelize").Op.not]: null } },
    });
    const productCount = await Product.count();
    const userCount = await User.count();

    console.log(
      "🎉 Database seeding completed successfully with exactly 300 products!"
    );
    console.log(`
📊 DATA SUMMARY:
