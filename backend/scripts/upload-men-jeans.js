...
const IMAGES_FOLDER = path.join(__dirname, '../upload/men-jeans');

async function uploadMenJeansImages() {
  console.log('🚀 Starting Men Jeans Image Upload Process...\n');
  ...
  try {
    ...
    if (files.length === 0) {
      console.log('❌ No images found in upload/men-jeans folder');
      console.log('💡 Please add your images to backend/upload/men-jeans/');
      return;
    }

    // Get products
    console.log('\n🔍 Finding men jeans products...');
    const [products] = await connection.execute(`
      ...
      WHERE sc.name = 'Men Jeans' AND p.isActive = 1
      ORDER BY p.id ASC
      LIMIT 20
    `);
    ...
    if (products.length === 0) {
      console.log('❌ No men jeans products found in database');
      console.log('💡 Make sure you have products in the "Men Jeans" subcategory');
      return;
    }
    ...
    for (let i = 0; i < Math.min(products.length, files.length); i++) {
      ...
      try {
        ...
        const uploadResult = await cloudinary.uploader.upload(filePath, {
          folder: 'clothing-store/men-jeans',
          public_id: `men-jeans-${file.split('.')[0]}-${Date.now()}`,
          transformation: [
    ...
    console.log(`📁 Cloudinary folder: clothing-store/men-jeans`);
    ...
    console.log('🌐 Your men jeans products now have Cloudinary image URLs!');
    console.log('🔍 Test by searching for "men jeans" on your website.');
...
if (require.main === module) {
  uploadMenJeansImages();
}

module.exports = { uploadMenJeansImages };