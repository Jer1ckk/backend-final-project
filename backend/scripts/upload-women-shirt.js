...
const IMAGES_FOLDER = path.join(__dirname, '../upload/women-shirt');

async function uploadWomenShirtImages() {
  console.log('🚀 Starting Women Shirt Image Upload Process...\n');
  ...
  try {
    ...
    if (files.length === 0) {
      console.log('❌ No images found in upload/women-shirt folder');
      console.log('💡 Please add your images to backend/upload/women-shirt/');
      return;
    }

    // Get products
    console.log('\n🔍 Finding women shirt products...');
    const [products] = await connection.execute(`
      ...
      WHERE sc.name = 'Women Shirt' AND p.isActive = 1
      ORDER BY p.id ASC
      LIMIT 20
    `);
    ...
    if (products.length === 0) {
      console.log('❌ No women shirt products found in database');
      console.log('💡 Make sure you have products in the "Women Shirt" subcategory');
      return;
    }
    ...
    for (let i = 0; i < Math.min(products.length, files.length); i++) {
      ...
      try {
        ...
        const uploadResult = await cloudinary.uploader.upload(filePath, {
          folder: 'clothing-store/women-shirt',
          public_id: `women-shirt-${file.split('.')[0]}-${Date.now()}`,
          transformation: [
    ...
    console.log(`📁 Cloudinary folder: clothing-store/women-shirt`);
    ...
    console.log('🌐 Your women shirt products now have Cloudinary image URLs!');
    console.log('🔍 Test by searching for "women shirt" on your website.');
...
if (require.main === module) {
  uploadWomenShirtImages();
}

module.exports = { uploadWomenShirtImages };