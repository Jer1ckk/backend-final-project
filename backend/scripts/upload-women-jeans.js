...
const IMAGES_FOLDER = path.join(__dirname, '../upload/women-jeans');

async function uploadWomenJeansImages() {
  console.log('🚀 Starting Women Jeans Image Upload Process...\n');
  ...
  try {
    ...
    if (files.length === 0) {
      console.log('❌ No images found in upload/women-jeans folder');
      console.log('💡 Please add your images to backend/upload/women-jeans/');
      return;
    }

    // Get products
    console.log('\n🔍 Finding women jeans products...');
    const [products] = await connection.execute(`
      ...
      WHERE sc.name = 'Women Jeans' AND p.isActive = 1
      ORDER BY p.id ASC
      LIMIT 20
    `);
    ...
    if (products.length === 0) {
      console.log('❌ No women jeans products found in database');
      console.log('💡 Make sure you have products in the "Women Jeans" subcategory');
      return;
    }
    ...
    for (let i = 0; i < Math.min(products.length, files.length); i++) {
      ...
      try {
        ...
        const uploadResult = await cloudinary.uploader.upload(filePath, {
          folder: 'clothing-store/women-jeans',
          public_id: `women-jeans-${file.split('.')[0]}-${Date.now()}`,
          transformation: [
    ...
    console.log(`📁 Cloudinary folder: clothing-store/women-jeans`);
    ...
    console.log('🌐 Your women jeans products now have Cloudinary image URLs!');
    console.log('🔍 Test by searching for "women jeans" on your website.');
...
if (require.main === module) {
  uploadWomenJeansImages();
}

module.exports = { uploadWomenJeansImages };