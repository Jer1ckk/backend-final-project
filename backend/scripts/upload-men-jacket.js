...
const IMAGES_FOLDER = path.join(__dirname, '../upload/men-jacket');

async function uploadMenJacketImages() {
  console.log('🚀 Starting Men Jacket Image Upload Process...\n');
  ...
  try {
    ...
    if (files.length === 0) {
      console.log('❌ No images found in upload/men-jacket folder');
      console.log('💡 Please add your images to backend/upload/men-jacket/');
      return;
    }

    // Get products
    console.log('\n🔍 Finding men jacket products...');
    const [products] = await connection.execute(`
      ...
      WHERE sc.name = 'Men Jacket' AND p.isActive = 1
      ORDER BY p.id ASC
      LIMIT 20
    `);
    ...
    if (products.length === 0) {
      console.log('❌ No men jacket products found in database');
      console.log('💡 Make sure you have products in the "Men Jacket" subcategory');
      return;
    }
    ...
    for (let i = 0; i < Math.min(products.length, files.length); i++) {
      ...
      try {
        ...
        const uploadResult = await cloudinary.uploader.upload(filePath, {
          folder: 'clothing-store/men-jacket',
          public_id: `men-jacket-${file.split('.')[0]}-${Date.now()}`,
          transformation: [
    ...
    console.log(`📁 Cloudinary folder: clothing-store/men-jacket`);
    ...
    console.log('🌐 Your men jacket products now have Cloudinary image URLs!');
    console.log('🔍 Test by searching for "men jacket" on your website.');
...
if (require.main === module) {
  uploadMenJacketImages();
}

module.exports = { uploadMenJacketImages };