...
const IMAGES_FOLDER = path.join(__dirname, '../upload/women-trouser');

async function uploadWomenTrouserImages() {
  console.log('🚀 Starting Women Trouser Image Upload Process...\n');
  ...
  try {
    ...
    if (files.length === 0) {
      console.log('❌ No images found in upload/women-trouser folder');
      console.log('💡 Please add your images to backend/upload/women-trouser/');
      return;
    }

    // Get products
    console.log('\n🔍 Finding women trouser products...');
    const [products] = await connection.execute(`
      ...
      WHERE sc.name = 'Women Trouser' AND p.isActive = 1
      ORDER BY p.id ASC
      LIMIT 20
    `);
    ...
    if (products.length === 0) {
      console.log('❌ No women trouser products found in database');
      console.log('💡 Make sure you have products in the "Women Trouser" subcategory');
      return;
    }
    ...
    for (let i = 0; i < Math.min(products.length, files.length); i++) {
      ...
      try {
        ...
        const uploadResult = await cloudinary.uploader.upload(filePath, {
          folder: 'clothing-store/women-trouser',
          public_id: `women-trouser-${file.split('.')[0]}-${Date.now()}`,
          transformation: [
    ...
    console.log(`📁 Cloudinary folder: clothing-store/women-trouser`);
    ...
    console.log('🌐 Your women trouser products now have Cloudinary image URLs!');
    console.log('🔍 Test by searching for "women trouser" on your website.');
...
if (require.main === module) {
  uploadWomenTrouserImages();
}

module.exports = { uploadWomenTrouserImages };