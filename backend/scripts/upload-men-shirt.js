...
const IMAGES_FOLDER = path.join(__dirname, '../upload/men-shirt');

async function uploadMenShirtImages() {
  console.log('🚀 Starting Men Shirt Image Upload Process...\n');
  ...
  try {
    ...
    if (files.length === 0) {
      console.log('❌ No images found in upload/men-shirt folder');
      console.log('💡 Please add your images to backend/upload/men-shirt/');
      return;
    }

    // Get products
    console.log('\n🔍 Finding men shirt products...');
    const [products] = await connection.execute(`
      ...
      WHERE sc.name = 'Men Shirt' AND p.isActive = 1
      ORDER BY p.id ASC
      LIMIT 20
    `);
    ...
    if (products.length === 0) {
      console.log('❌ No men shirt products found in database');
      console.log('💡 Make sure you have products in the "Men Shirt" subcategory');
      return;
    }
    ...
    for (let i = 0; i < Math.min(products.length, files.length); i++) {
      ...
      try {
        ...
        const uploadResult = await cloudinary.uploader.upload(filePath, {
          folder: 'clothing-store/men-shirt',
          public_id: `men-shirt-${file.split('.')[0]}-${Date.now()}`,
          transformation: [
    ...
    console.log(`📁 Cloudinary folder: clothing-store/men-shirt`);
    ...
    console.log('🌐 Your men shirt products now have Cloudinary image URLs!');
    console.log('🔍 Test by searching for "men shirt" on your website.');
...
if (require.main === module) {
  uploadMenShirtImages();
}

module.exports = { uploadMenShirtImages };