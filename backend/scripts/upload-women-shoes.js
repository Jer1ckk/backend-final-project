...
const IMAGES_FOLDER = path.join(__dirname, '../upload/women-shoes');

async function uploadWomenShoesImages() {
  console.log('🚀 Starting Women Shoes Image Upload Process...\n');
  ...
  try {
    ...
    if (files.length === 0) {
      console.log('❌ No images found in upload/women-shoes folder');
      console.log('💡 Please add your images to backend/upload/women-shoes/');
      return;
    }

    // Get products
    console.log('\n🔍 Finding women shoes products...');
    const [products] = await connection.execute(`
      ...
      WHERE sc.name = 'Women Shoes' AND p.isActive = 1
      ORDER BY p.id ASC
      LIMIT 20
    `);
    ...
    if (products.length === 0) {
      console.log('❌ No women shoes products found in database');
      console.log('💡 Make sure you have products in the "Women Shoes" subcategory');
      return;
    }
    ...
    for (let i = 0; i < Math.min(products.length, files.length); i++) {
      ...
      try {
        ...
        const uploadResult = await cloudinary.uploader.upload(filePath, {
          folder: 'clothing-store/women-shoes',
          public_id: `women-shoes-${file.split('.')[0]}-${Date.now()}`,
          transformation: [
    ...
    console.log(`📁 Cloudinary folder: clothing-store/women-shoes`);
    ...
    console.log('🌐 Your women shoes products now have Cloudinary image URLs!');
    console.log('🔍 Test by searching for "women shoes" on your website.');
...
if (require.main === module) {
  uploadWomenShoesImages();
}

module.exports = { uploadWomenShoesImages };