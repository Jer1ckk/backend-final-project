...
const IMAGES_FOLDER = path.join(__dirname, '../upload/women-jacket');

async function uploadWomenJacketImages() {
  console.log('🚀 Starting Women Jacket Image Upload Process...\n');
  ...
  try {
    ...
    if (files.length === 0) {
      console.log('❌ No images found in upload/women-jacket folder');
      console.log('💡 Please add your images to backend/upload/women-jacket/');
      return;
    }

    // Get products
    console.log('\n🔍 Finding women jacket products...');
    const [products] = await connection.execute(`
      ...
      WHERE sc.name = 'Women Jacket' AND p.isActive = 1
      ORDER BY p.id ASC
      LIMIT 20
    `);
    ...
    if (products.length === 0) {
      console.log('❌ No women jacket products found in database');
      console.log('💡 Make sure you have products in the "Women Jacket" subcategory');
      return;
    }
    ...
    for (let i = 0; i < Math.min(products.length, files.length); i++) {
      ...
      try {
        ...
        const uploadResult = await cloudinary.uploader.upload(filePath, {
          folder: 'clothing-store/women-jacket',
          public_id: `women-jacket-${file.split('.')[0]}-${Date.now()}`,
          transformation: [
    ...
    console.log(`📁 Cloudinary folder: clothing-store/women-jacket`);
    ...
    console.log('🌐 Your women jacket products now have Cloudinary image URLs!');
    console.log('🔍 Test by searching for "women jacket" on your website.');
...
if (require.main === module) {
  uploadWomenJacketImages();
}

module.exports = { uploadWomenJacketImages };