...
const IMAGES_FOLDER = path.join(__dirname, '../upload/women-tshirt');

async function uploadWomenTshirtImages() {
  console.log('🚀 Starting Women T-shirt Image Upload Process...\n');
  ...
  try {
    ...
    if (files.length === 0) {
      console.log('❌ No images found in upload/women-tshirt folder');
      console.log('💡 Please add your images to backend/upload/women-tshirt/');
      return;
    }

    // Get products
    console.log('\n🔍 Finding women t-shirt products...');
    const [products] = await connection.execute(`
      ...
      WHERE sc.name = 'Women T-shirt' AND p.isActive = 1
      ORDER BY p.id ASC
      LIMIT 20
    `);
    ...
    if (products.length === 0) {
      console.log('❌ No women t-shirt products found in database');
      console.log('💡 Make sure you have products in the "Women T-shirt" subcategory');
      return;
    }
    ...
    for (let i = 0; i < Math.min(products.length, files.length); i++) {
      ...
      try {
        ...
        const uploadResult = await cloudinary.uploader.upload(filePath, {
          folder: 'clothing-store/women-tshirt',
          public_id: `women-tshirt-${file.split('.')[0]}-${Date.now()}`,
          transformation: [
    ...
    console.log(`📁 Cloudinary folder: clothing-store/women-tshirt`);
    ...
    console.log('🌐 Your women t-shirt products now have Cloudinary image URLs!');
    console.log('🔍 Test by searching for "women t-shirt" on your website.');
...
if (require.main === module) {
  uploadWomenTshirtImages();
}

module.exports = { uploadWomenTshirtImages };