...
const IMAGES_FOLDER = path.join(__dirname, '../upload/men-shoes');

async function uploadMenShoesImages() {
  console.log('🚀 Starting Men Shoes Image Upload Process...\n');
  ...
  try {
    ...
    if (files.length === 0) {
      console.log('❌ No images found in upload/men-shoes folder');
      console.log('💡 Please add your images to backend/upload/men-shoes/');
      return;
    }

    // Get products
    console.log('\n🔍 Finding men shoes products...');
    const [products] = await connection.execute(`
      ...
      WHERE sc.name = 'Men Shoes' AND p.isActive = 1
      ORDER BY p.id ASC
      LIMIT 20
    `);
    ...
    if (products.length === 0) {
      console.log('❌ No men shoes products found in database');
      console.log('💡 Make sure you have products in the "Men Shoes" subcategory');
      return;
    }
    ...
    for (let i = 0; i < Math.min(products.length, files.length); i++) {
      ...
      try {
        ...
        const uploadResult = await cloudinary.uploader.upload(filePath, {
          folder: 'clothing-store/men-shoes',
          public_id: `men-shoes-${file.split('.')[0]}-${Date.now()}`,
          transformation: [
    ...
    console.log(`📁 Cloudinary folder: clothing-store/men-shoes`);
    ...
    console.log('🌐 Your men shoes products now have Cloudinary image URLs!');
    console.log('🔍 Test by searching for "men shoes" on your website.');
...
if (require.main === module) {
  uploadMenShoesImages();
}

module.exports = { uploadMenShoesImages };