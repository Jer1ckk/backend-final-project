...
const IMAGES_FOLDER = path.join(__dirname, '../upload/women-shorts');

async function uploadWomenShortsImages() {
  console.log('🚀 Starting Women Shorts Image Upload Process...\n');
  ...
  try {
    ...
    if (files.length === 0) {
      console.log('❌ No images found in upload/women-shorts folder');
      console.log('💡 Please add your images to backend/upload/women-shorts/');
      return;
    }

    // Get products
    console.log('\n🔍 Finding women shorts products...');
    const [products] = await connection.execute(`
      ...
      WHERE sc.name = 'Women Shorts' AND p.isActive = 1
      ORDER BY p.id ASC
      LIMIT 20
    `);
    ...
    if (products.length === 0) {
      console.log('❌ No women shorts products found in database');
      console.log('💡 Make sure you have products in the "Women Shorts" subcategory');
      return;
    }
    ...
    for (let i = 0; i < Math.min(products.length, files.length); i++) {
      ...
      try {
        ...
        const uploadResult = await cloudinary.uploader.upload(filePath, {
          folder: 'clothing-store/women-shorts',
          public_id: `women-shorts-${file.split('.')[0]}-${Date.now()}`,
          transformation: [
    ...
    console.log(`📁 Cloudinary folder: clothing-store/women-shorts`);
    ...
    console.log('🌐 Your women shorts products now have Cloudinary image URLs!');
    console.log('🔍 Test by searching for "women shorts" on your website.');
...
if (require.main === module) {
  uploadWomenShortsImages();
}

module.exports = { uploadWomenShortsImages };