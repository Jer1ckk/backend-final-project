...
const IMAGES_FOLDER = path.join(__dirname, '../upload/men-trouser');

async function uploadMenTrouserImages() {
  console.log('🚀 Starting Men Trouser Image Upload Process...\n');
  ...
  try {
    ...
    if (files.length === 0) {
      console.log('❌ No images found in upload/men-trouser folder');
      console.log('💡 Please add your images to backend/upload/men-trouser/');
      return;
    }

    // Get products
    console.log('\n🔍 Finding men trouser products...');
    const [products] = await connection.execute(`
      ...
      WHERE sc.name = 'Men Trouser' AND p.isActive = 1
      ORDER BY p.id ASC
      LIMIT 20
    `);
    ...
    if (products.length === 0) {
      console.log('❌ No men trouser products found in database');
      console.log('💡 Make sure you have products in the "Men Trouser" subcategory');
      return;
    }
    ...
    for (let i = 0; i < Math.min(products.length, files.length); i++) {
      ...
      try {
        ...
        const uploadResult = await cloudinary.uploader.upload(filePath, {
          folder: 'clothing-store/men-trouser',
          public_id: `men-trouser-${file.split('.')[0]}-${Date.now()}`,
          transformation: [
    ...
    console.log(`📁 Cloudinary folder: clothing-store/men-trouser`);
    ...
    console.log('🌐 Your men trouser products now have Cloudinary image URLs!');
    console.log('🔍 Test by searching for "men trouser" on your website.');
...
if (require.main === module) {
  uploadMenTrouserImages();
}

module.exports = { uploadMenTrouserImages };